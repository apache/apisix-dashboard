...
context('Create and Delete Service ', () => {
  ...
  it('should duplicate the service', function () {
    ...
    // test view
    cy.contains(this.data.serviceName3).siblings().contains('View').click();
    cy.get(this.domSelector.drawer).should('be.visible');

    cy.get(this.domSelector.codemirrorScroll).within(() => {
      cy.contains('upstream').should('exist');
      cy.contains(this.data.serviceName3).should('exist');
    });
  });
  ...
  it('should delete the service', function () {
    ...
    const serviceNames = [this.data.serviceName2, this.data.serviceName3];
    serviceNames.forEach(function (serviceName) {
      cy.get(domSelector.name).clear().type(serviceName);
      cy.contains('Search').click();

      cy.contains(serviceName).siblings().contains('Delete').click();
      cy.contains('button', 'Confirm').click();
      cy.get(domSelector.notification).should('contain', data.deleteServiceSuccess);
    });
  });
});
