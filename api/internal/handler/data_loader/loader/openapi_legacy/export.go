...
func (o Loader) Export(data loader.DataSets) (interface{}, error) {
	...
	for i := range data.Routes {
		routes = append(routes, &data.Routes[i])
	}
	for i := range data.Upstreams {
		upstreams = append(upstreams, &data.Upstreams[i])
	}
	for i := range data.Services {
		services = append(services, &data.Services[i])
	}
...
func RouteToOpenAPI3() (*openapi3.Swagger, error) {
	paths := openapi3.Paths{}
	paramsRefs := make([]*openapi3.ParameterRef, 0)
	...
	for _, route := range routes {
		...
		//Parse upstream
		_upstream, err := ParseRouteUpstream(route)

		if err != nil {
			return nil, fmt.Errorf("parse route upstream failed: %v", err)
		...
		if err != nil {
			return nil, fmt.Errorf("parse labels failed: %v", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("parseRoutePlugins err: %v", err)
		}
...
func ParseRouteUpstream(route *entity.Route) (interface{}, error) {
