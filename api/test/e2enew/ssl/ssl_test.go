...
var _ = ginkgo.Describe("SSL Basic", func() {
	...
	ginkgo.It("without certificate", func() {
		...
		gomega.Expect(err.Error()).Should(gomega.ContainSubstring("remote error: tls: internal error"))
	})
	...
	ginkgo.It("test disable SSL HTTPS request", func() {
		...
		_, err := http.Get("https://www.test2.com:9443")
		gomega.Expect(fmt.Sprintf("%s", err)).Should(gomega.ContainSubstring("remote error: tls: internal error"))
	})
