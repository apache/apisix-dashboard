...
Currently we reject import duplicate route, that is to say when you import a route which has the same attributes, all of the `uri` `uris` `host` `hosts` `remote_addr` `remote_addrs` `priority` `vars` `filter_func`, as the existing route, you will get an error while importing a route from OAS3.0.
