...
func cHashKeySchemaCheck(upstream *entity.UpstreamDef) error {
	...
	if upstream.HashOn == "header" || upstream.HashOn == "cookie" {
		...
		if schemaDef == "" {
			return fmt.Errorf("schema validate faileded: schema not found, path: main.upstream_hash_header_schema")
		}
	}

	s, err := gojsonschema.NewSchema(gojsonschema.NewStringLoader(schemaDef))
	if err != nil {
		return fmt.Errorf("schema validate faileded: %w", err)
	}
	...
	if !ret.Valid() {
		...
		return fmt.Errorf("schema validate faileded: %s", errString.String())
	}
...
func (v *APISIXJsonSchemaValidator) Validate(obj interface{}) error {
	ret, err := v.schema.Validate(gojsonschema.NewGoLoader(obj))
	if err != nil {
		log.Warnf("schema validate faileded: %w", err)
		return fmt.Errorf("schema validate faileded: %w", err)
	}
	...
	for pluginName, pluginConf := range plugins {
		...
		if schemaDef == "" {
			log.Warnf("schema validate faileded: schema not found, path: %s", "plugins."+pluginName)
			return fmt.Errorf("schema validate faileded: schema not found, path: %s", "plugins."+pluginName)
		}

		schemaDef = reg.ReplaceAllString(schemaDef, `"properties":{}`)
		s, err := gojsonschema.NewSchema(gojsonschema.NewStringLoader(schemaDef))
		if err != nil {
			log.Warnf("init schema validate faileded: %w", err)
			return fmt.Errorf("schema validate faileded: %w", err)
		}
