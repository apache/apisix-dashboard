...
func TestRoute(t *testing.T) {
	...
	reqBodyErr := `{
		"id": 1,
		` + reqBodyCopy
	errRoute2 := &UpdateInput{}
	errRoute2.ID = "2"
	err = json.Unmarshal([]byte(reqBodyErr), errRoute2)
	assert.Nil(t, err)
	ctx.SetInput(errRoute2)
	ret, err = handler.Update(ctx)
	assert.NotNil(t, err)
	assert.EqualError(t, err, "ID on path (2) doesn't match ID on body (%!s(float64=1))")
	assert.Equal(t, http.StatusBadRequest, ret.(*data.SpecCodeResponse).StatusCode)
