...
func TestSSL_Patch_Method(t *testing.T) {
	// init
	err := storage.InitETCDClient(conf.ETCDConfig)
	assert.Nil(t, err)
	err = store.InitStores()
	...
	assert.Nil(t, err)
	assert.Equal(t, stored.ID, ssl.ID)
	assert.Equal(t, stored.Labels["build"], "28")
	assert.Equal(t, stored.Labels["env"], "dev1")
	assert.Equal(t, stored.Labels["version"], "v2")
...
func TestSSL_Patch_Path_Method(t *testing.T) {
	// init
	err := storage.InitETCDClient(conf.ETCDConfig)
	assert.Nil(t, err)
	err = store.InitStores()
