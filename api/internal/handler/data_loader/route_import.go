...
func (h *ImportHandler) Import(c droplet.Context) (interface{}, error) {
	...
	case LoaderTypeOpenAPI3:
		...
		break
	default:
		return nil, fmt.Errorf("unsupported data loader type")
	}
	...
	errs := h.createEntities(dataSets)
	return h.convertToImportResult(dataSets, errs), nil
}

// create imported resources
func (h *ImportHandler) createEntities(data *loader.DataSets) map[store.HubKey][]string {
	errs := make(map[store.HubKey][]string)

	for _, route := range data.Routes {
		_, err := h.routeStore.Create(context.TODO(), &route)
		if err != nil {
	...
	for _, upstream := range data.Upstreams {
		_, err := h.upstreamStore.Create(context.TODO(), &upstream)
		if err != nil {
	...
	for _, service := range data.Services {
		_, err := h.serviceStore.Create(context.TODO(), &service)
		if err != nil {
	...
	for _, consumer := range data.Consumers {
		_, err := h.consumerStore.Create(context.TODO(), &consumer)
		if err != nil {
	...
	for _, ssl := range data.SSLs {
		_, err := h.sslStore.Create(context.TODO(), &ssl)
		if err != nil {
	...
	for _, route := range data.StreamRoutes {
		_, err := h.streamRouteStore.Create(context.TODO(), &route)
		if err != nil {
	...
	for _, plugin := range data.GlobalPlugins {
		_, err := h.globalPluginStore.Create(context.TODO(), &plugin)
		if err != nil {
	...
	for _, config := range data.PluginConfigs {
		_, err := h.pluginConfigStore.Create(context.TODO(), &config)
		if err != nil {
	...
	for _, proto := range data.Protos {
		_, err := h.protoStore.Create(context.TODO(), &proto)
		if err != nil {
